...
export default function BlogsList({ dict, lang = 'en', limit, titles, empty }) {
    ...
    const filteredTitles = useMemo(() => {
        return titles.filter((title) =>
            title.title.toLowerCase().includes(query.toLowerCase()) // Match query with the title (case insensitive)
        );
    }, [query, titles]);
        ...
        <>
            ...
            <span className="mb-15">
                {searchedCount > 0
                    ...
                    : query ? 'No results found for your search.' : 'Showing all results.'
                }
            </span>
                        ...
                        : (empty ? "No Result Found" :
                            Array(5).fill(' ').map((_, index) => (
                                <div key={index}>
                                    ...
                                    <div className="skeleton" style={{ width: '100%', height: '40px', margin: '16px 0' }}></div>
                                    <div className="skeleton" style={{ width: '60%', height: '24px', margin: '8px 0' }}></div>
                                </div>
                            ))
                        )
                }
