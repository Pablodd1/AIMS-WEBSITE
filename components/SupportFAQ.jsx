    ...
    <section className="my-12 px-2 ">
        ...
        {Array.from({ length: loadedQuestions }).map((_, index) => (
          <li key={index} className="flex w-full">
            <div className="h-fit my-2">
              <button
                className={`flex items-center justify-between w-full px-4 py-3.5 border-b border-secondary focus:outline-none ${
                  expanded === index ? "" : ""
                }`}
              ...
              >
                ...
                <span
                  className={`ml-2 my-animi ${expanded === index ? "-rotate-180" : ""}`}
                >
                  <FcExpand />
                </span>
              </button>
              <div
                className={`bg-gray-300 overflow-hidden text-gray-700 text-sm md:text-md transition-all duration-700 rounded-xl ${
                  expanded === index ? "h-42 py-10 px-5 mx-2 my-5" : "h-0"
                }`}
              >
                <span className="font-semibold uppercase tracking-wide  underline">
                  Answer:
                </span>
      ...
      {loadedQuestions < faqs && (
        <SecondaryButton
          ...
          onClick={() =>
            setLoadedQuestions((prev) => (prev + 3 >= faqs ? faqs : prev + 3))
          }
          withArrow
