    ...
    return (
        <div className=" w-11/12 xl:max-w-7xl grid grid-cols-2 md:grid-cols-3 grid-rows-2 sm:grid-rows-3 md:grid-rows-2 gap-4 max-w-7xl mx-auto my-24" >
            {videos.map((video, index) => (
                <div
                    key={video.id}
                    className={`h-full flex flex-col justify-center ${index === 0 ? 'row-span-1 sm:row-span-2 md:row-span-2 col-span-2 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500' : 'row-span-1'
                        }`}
                >
                    <iframe
                        width="100%"
                    ...
                    <div className="video-info">
                        ...
                        <p className='text-gray-600 font-sans' >{video.description}
                            <Link href={video.more} target="_blank" className={"m-1 px-3 text-primary font-bold uppercase text-xs rounded-md hover:underline "} >
                                Read More
                            </Link></p>
                    </div>
                </div>
