import { langAbout } from "@dictionary/ABOUT/main";
...
const startWhatsAppChat = () => {
  ...
  const message = encodeURIComponent(
    "Hello Sir, I'm interested in your AI Medical Scriber. Can we talk about this?",
  );
    ...
    <>
      ...
      <section className="my-32 px-2 mx-auto max-w-fit sm:max-w-2xl  md:max-w-4xl xl:max-w-6xl">
        <article className="  my-6 mx-auto w-4/5  ">
          ...
          <p className="">{mv.vision}</p>
          <br />
          <h1 className="text-4xl font-semibold font-sans my-2 ">
            {mv.mission_h}
          </h1>
          <p className="">{mv.mission}</p>
          <br />
          <PremiumButton
            label={mv.btn}
          ...
          {(await langAbout(lang, "contact")).map((x, i) => {
            return (
              ...
              >
                ...
                <p className=" text-sm lg:text-md">{x.msg}</p>
                <SecondaryButton
                  label={`${x.contact}`}
                  href={x.action}
                  withArrow
